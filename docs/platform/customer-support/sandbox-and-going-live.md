...
- **Every feature is unlocked**, regardless of the plan you were invited on. WhatsApp configuration, tool packs, the full agent runtime — a Free-invited org can fully evaluate Scale-level behaviour before committing. The one exception is the **"Powered by Lira" attribution**, which shows in sandbox on every plan: removing it needs a paid plan *and* production traffic.
...
- **No email leaves the building — including to your own team.** Ticket and escalation notifications are suppressed in test mode too. You verify the flow in the dashboard: the ticket, its escalation reason and its timeline all appear exactly as they would live. This is deliberate — a live workspace running a staging integration must not page its support team for test traffic.
